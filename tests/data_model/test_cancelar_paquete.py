...
def test_cancelar_desde_recibido(db_session):
    ...
    cancel(db_session, p, op, MotivoCancelacion.DEVUELTO_AL_TRANSPORTADOR)

    assert p.estado == EstadoPaquete.CANCELADO
    assert p.cancel_reason == "DEVUELTO_AL_TRANSPORTADOR"
...
def test_cancelar_un_entregado_se_rechaza_sin_efecto(db_session):
    ...
    with pytest.raises(TransicionInvalida):
        cancel(db_session, p, op, MotivoCancelacion.OTRO)
...
def test_cancelar_dos_veces_se_rechaza_sin_efecto(db_session):
    op = _usuario(db_session)
    p = _anunciar(db_session)
    cancel(db_session, p, op, MotivoCancelacion.NO_RECLAMADO)
    ...
    assert p.estado == EstadoPaquete.CANCELADO
    assert p.cancel_reason == reason_antes == "NO_RECLAMADO"
